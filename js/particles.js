...
class Particle {
  ...
  update() {
    ...
    if (mouse.x) {
      ...
      if(dist < 120){ this.x += dx/dist; this.y += dy/dist; }
    }
  ...
  draw() {
    ...
    ctx.shadowBlur=15; ctx.shadowColor=this.color; ctx.globalAlpha=this.alpha;
...
function animate() {
  ...
  for(let i=0;i<particles.length;i++){
    ...
    for(let j=i+1;j<particles.length;j++){
      ...
      if(dist<130){ ctx.beginPath(); ctx.strokeStyle=`rgba(0,255,255,${1-dist/130})`;
